...
BEGIN
	...
	b.gid original_gid, ST_CollectionExtract(ST_Intersection(ST_SETSRID(ST_Force2d(b.valid_geom), 4326), ST_SETSRID(ST_Force2d(a.geom), 4326)), 3) geom FROM grid.adm2_overlay a
	LEFT JOIN dados_brutos.%I b ON ST_Intersects(ST_SETSRID(ST_Force2d(a.geom), 4326), ST_SETSRID(ST_Force2d(b.valid_geom), 4326)) WHERE cd_grid = %s AND am_legal %s', 
