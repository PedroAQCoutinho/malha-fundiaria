...
BEGIN
	...
	LEFT JOIN dados_brutos.%I b ON ST_Intersects(ST_SETSRID(ST_Force2d(ST_CollectionExtract(ST_Makevalid(a.geom),3)), 4326), ST_SETSRID(ST_Force2d(ST_CollectionExtract(b.valid_geom,3)), 4326)) WHERE cd_grid = %s AND NOT am_legal AND substring(a.cd_mun::TEXT, 1, 2)::int IN (22, 31) %s', 
...
BEGIN
   EXECUTE format('CREATE TEMPORARY TABLE griddata_%s AS 
				...
				FROM grid.adm2_overlay ao WHERE cd_grid = %s and not am_legal AND substring(ao.cd_mun::TEXT, 1, 2)::int IN (22, 31) ) foo;', 
     grid, grid, grid);
END
$func$;
