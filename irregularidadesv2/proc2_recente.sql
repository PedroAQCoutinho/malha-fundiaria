...
SELECT id_car_original, sum(CASE WHEN desmatamento >= 9 THEN area ELSE 0 END) area_desmatamento_recente,  
CASE WHEN sum(CASE WHEN desmatamento >= 9 THEN area ELSE 0 END) = 0 THEN FALSE ELSE TRUE END is_recente FROM (
...
WHERE cat_fund IS NOT NULL AND pm.is_car AND desmatamento IS NOT NULL 
