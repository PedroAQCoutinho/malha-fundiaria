...
SELECT-- Feições de todo o resto subtraído 
	...
	END geom
FROM 
	outputs.step15_overlay a 
LEFT JOIN 
	(SELECT gid, (st_dump(geom)).geom FROM inputs.inputs_faixa_fronteira) b
ON
...
FROM 
	(SELECT * FROM inputs.inputs_faixa_fronteira) a	
LEFT JOIN 
	(SELECT * FROM outputs.step15_overlay) b 
ON
	ST_Intersects(a.geom, b.geom)
WHERE 
	(a.gid % :var_num_proc) = :var_proc;